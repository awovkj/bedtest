...
export async function initializeChunkedUpload(context) {
    ...
    try {
        ...
        const sessionInfo = {
            ...
            status: 'initialized',
            createdAt: timestamp,
            expiresAt: timestamp + 86400000 // 24小时过期
        };
...
export async function handleChunkUpload(context) {
    ...
    try {
        ...
        const initialChunkMetadata = {
            ...
            status: 'uploading',
            uploadChannel,
            timeoutThreshold: uploadStartTime + 900000 // 15分钟超时阈值
        };
...
// 重试单个失败的分块
async function retrySingleChunk(context, chunk, uploadChannel, maxRetries = 5, retryTimeout = 300000) {
    const { env } = context;
    const db = getDatabase(env);
